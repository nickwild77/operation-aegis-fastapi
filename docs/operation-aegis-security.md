...
| Secret scanning | Gitleaks | Pull requests, pushes to `main`, weekly schedule, manual runs | Committed secret detected | SARIF artifact, Code Scanning upload, PR comment |
...
For pull requests it scans the pull request commit range so historical baseline
findings do not block unrelated changes. Push, scheduled, and manual runs scan
the repository history.

## Feedback and reporting

Developers receive feedback in four places:

- GitHub check status on the pull request.
- Pull request comments from Dependency Review, Gitleaks, and the Operation
  Aegis security summary.
- GitHub Actions job summaries and uploaded artifacts.
